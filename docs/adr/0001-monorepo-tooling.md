...
- Active shared packages: `@shared/env` (`constants` / `gateway` / `api` / `web`), `@shared/http` (`filters` / `interceptors` / `auth` / `middleware` / `swagger`), `@shared/api-client`, `@shared/ui` (prefer `@shared/ui/components` + `theme.css` — [frontend.md](../frontend.md)), `@shared/types`, `@shared/config` (`eslint/*`).

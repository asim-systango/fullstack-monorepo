...
export class AuthService {
  ...
  async updateMe(userId: string, dto: UpdateMeDto) {
    ...
    if (updated.role === 'user') {
      try {
        await this.domainApi.syncMemberProfile(userId, {
          email: updated.email,
  ...
  private async ensureActiveMemberProfile(user: User): Promise<void> {
    let profile = await this.domainApi.getMemberProfile(user.id);

    if (!profile) {
      try {
        profile = await this.domainApi.provisionMemberProfile({
          userId: user.id,
          email: user.email,
          fullName: user.name,
        });

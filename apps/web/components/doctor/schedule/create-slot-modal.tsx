...
import { useCreateBulkSlots } from '@/features/slot/hooks';
...
export function CreateSlotModal({
  ...
  // Custom API mutation hook for bulk slot generation
  const createBulk = useCreateBulkSlots();

  // Estimate total slots for bulk mode
  ...
  }, [startDate, endDate, frequency, daysOfWeek, shifts, durationMinutes]);
  ...
  const isLoading = createBulk.isPending;
  const rawError = createBulk.error as {
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-0.5">
                Configure consultation slots using multi-shift generator
              </p>
            </div>
          </div>
          <button
